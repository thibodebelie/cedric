...
def solve_tssp(scenario_ids):
    ...
    model = gp.Model("TSSP_Model_2_Reliability")
    ...
    # eta[sid]: auxiliary variable for realized max handling time per scenario
    ...
    eta = {}  
    for sid in scenario_ids:
        # Define eta[sid] as the max handling time for this scenario
        eta[sid] = model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"Max_Realized_HT_{sid}")
        ...
        for f in FLIGHT_LIST:
            ...
            # Linearization: eta[sid] must be >= handling time of every flight in scenario sid
            model.addConstr(eta[sid] >= (ST_real[sid, f, "Step_3_Infeed"] + data_omega["Step_3_Infeed"] - data_omega["arrival"]))
        ...
        # Precendence and slack constraints
    ...
    # Objective Function
    # Part A: Expected max realized handling time (E[eta])
    expected_max_ht = (1.0 / num_scenarios) * gp.quicksum(eta[sid] for sid in scenario_ids)
    
    # Part B: Expected recourse penalty
    abs_delta = {}
    for sid in scenario_ids:
        for f in FLIGHT_LIST:
    ...
    expected_recourse_penalty = (1.0 / num_scenarios) * gp.quicksum(abs_delta[sid, f] for sid in scenario_ids for f in FLIGHT_LIST)
    
    model.setObjective(expected_max_ht + (LAMBDA_STABILITY * expected_recourse_penalty), GRB.MINIMIZE)
